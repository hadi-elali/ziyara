...
- Admins können eigenständige App-Konten zu benannten Kontofamilien zusammenfassen. Ein Profil gehört höchstens einer Kontofamilie; eine neue Zuordnung verschiebt es atomar aus der bisherigen Familie. Diese Zuordnung ist unabhängig von `party_size` und Reisegruppen und kann unter `Reiseorganisation → Zwischenreise & Busse` als gemeinsame Zuordnungseinheit verwendet werden.
...
- `medical_staff` und `organization_team` besitzen zusätzlich ein rollenbezogenes Notfall-Postfach. Jedes Konto dieser Rollen ist automatisch und dauerhaft für passende Notfälle aktiv; Admins erhalten beide Notfallarten. Eine manuelle Diensteinteilung existiert im aktuellen Client nicht. Andere Reise- und Adminrechte bleiben davon unberührt.

### Reise- und Inhaltsfunktionen

- Startseite mit einer kompakten Vorschau des heutigen Tagesprogramms für angemeldete Nutzer sowie wichtigen Städten und hervorgehobenen Orten; die Vorschau öffnet eine gegliederte Sieben-Tage-Ansicht. Medizinisches Personal, Organisationsteam und Admins sehen dort außerdem einen hervorgehobenen Hinweis, solange ihr persönliches Notfall-Postfach mindestens eine ungelesene Meldung enthält; Admins erhalten beide Notfallarten.
...
- Beim ersten Start erscheint nach dem nativen Splash Screen direkt das Onboarding, während das lokal gebündelte Einführungsvideo mit Ton in Endlosschleife im Hintergrund läuft. Logo, Markentext und Sprachauswahl werden gemeinsam nach 1,65 Sekunden weich eingeblendet; die Animation verändert ausschließlich die Deckkraft. Ein Tipp auf eine Sprache speichert sie und öffnet direkt die Registrierung. Dort bleiben Anmeldung und die Nutzung des öffentlichen Guides ohne Konto als gleichwertige Wege erreichbar.
...
- Angemeldete Nutzer erreichen die geschützte Route `/emergency` ausschließlich von der Home-/Indexseite über einen kompakten, runden Alarm-Button rechts auf halber Bildschirmhöhe. Dort wählen Nutzer ausdrücklich das medizinische Team oder Reiseteam, beschreiben Anliegen und Aufenthaltsort und können optional nach Vordergrund-Berechtigung genau eine Geräteposition mitsenden. Es gibt kein Live- oder Hintergrundtracking. Die Datenbank materialisiert ausnahmslos alle zum Absendezeitpunkt vorhandenen Profile der Rolle `medical_staff` beziehungsweise `organization_team` sowie alle Profile der Rolle `admin` als Empfänger; Admins erhalten dadurch beide Notfallarten, und frühere Einteilungsdaten werden nicht berücksichtigt. Die Meldung bleibt für diese Empfänger im Postfach lesbar und besitzt einen individuellen Lesestatus.
...
- Der Home-Hinweis für das zuständige medizinische Personal, Organisationsteam und alle als Empfänger materialisierten Admins wird aus dem individuellen Empfänger- und Lesestatus aufgebaut, über Realtime sowie App-Rückkehr aktualisiert und verschwindet, sobald alle eigenen Meldungen als gelesen markiert sind. Ein gestaffelter Fallback-Refresh hält die Anzeige auch bei ausbleibendem Realtime-Ereignis aktuell.

- Die Reiseorganisation modelliert ausdrücklich kleine Zwischenreisen innerhalb der gesamten Ziyara, nicht die Gesamtreise selbst. Ein Admin kann eine aktive Zwischenreise wie `Hotel → Karbala` mit benannten Bussen anlegen. Jeder neu angelegte Bus benötigt eine registrierte Person als Busführer. Anschließend wird entweder eine registrierte Person oder eine ganze Kontofamilie einem Bus zugeordnet; Führer-, Personen- und Familienergebnisse erscheinen erst nach einer Texteingabe, und nach der Familienzuordnung werden deren Mitglieder nicht zusätzlich in der Einzelpersonenauswahl angeboten.
- Die aktive Zwischenreise ist im Adminbereich zuklappbar und kann ohne offenes Boarding beendet werden. Danach lässt sich eine leere Zwischenreise anlegen oder die Busanordnung einer beendeten Zwischenreise vollständig übernehmen: Busse, Busführer und Personen-/Familienzuordnungen werden kopiert, Boardingstände und andere Funktionen nicht.
- Im gemeinsamen Admin-Punkt `Reiseorganisation` stellt ein Admin im Schritt `Gruppen` die der Zwischenreise zugeordneten Personen zu Untergruppen zusammen und bestimmt genau eine registrierte Person als Gruppenanführer. Die Auswahlfelder zeigen Namen erst nach einer Suche; gewählte Mitglieder bleiben separat sichtbar und entfernbar. Eine Person gehört höchstens einer Untergruppe; der Anführer ist immer zugleich Mitglied. Gruppen können atomar geändert oder gelöscht werden.
...
- Der Adminbereich bündelt Zwischenreise, Busse, Untergruppen, Reiseführung und Generalalarm im Akkordeon `Reiseorganisation`. Darin bleiben die Schritte `Zwischenreise & Busse`, `Gruppen` und `Live-Begleitung` klar getrennt; eine gemeinsame Übersicht zeigt aktive Zwischenreise, Busse, Teilnehmer, Gruppen, Reiseführung und Alarmstatus. Im Schritt `Live-Begleitung` legt der Admin für den Generalalarm Meldung und eine frei wählbare Abfahrtszeit zwischen 1 und 1440 Minuten fest. Die Werte 15, 30 und 60 Minuten bleiben als Schnellauswahl verfügbar. Anschließend schaltet der Admin den Alarm ausdrücklich ein, überwacht ihn und beendet ihn wieder.
...
8. **Busmanagement:** Zwischenreise-, Bus-, Personen-/Familienzuordnungs- und Boardingmodell, verpflichtende Busführer, zuklappbare und beendbare Zwischenreisen sowie optionale Übernahme einer beendeten Busanordnung sind implementiert. Ein interner Legacy-Schlüssel bleibt nur zur relationalen Kompatibilität bestehen und wird weder eingegeben noch angezeigt. RLS/RPCs, Realtime, monotone Request-Versionen, Parallelitätstests und E2E-Smoke der bisherigen Boardinglogik bleiben erhalten. Der Session-Retry erneuert bei Auth-/Function-Grant-Fehlern die Session und wiederholt den Status-RPC genau einmal für dieselbe User-ID.
...
10. **Tagesprogramm:** von Zwischenreisen unabhängige, datumsbasierte Ein- und Mehrtagesplanung für die gesamte Ziyara, atomare Admin-Upserts, Realtime-/Fokus-/Fallback-Synchronisierung, benutzergebundener Neustart-Cache, lokale Datumsberechnung ohne UTC-Tagesverschiebung sowie Home-Vorschau und Wochenansicht für alle angemeldeten Nutzer sind implementiert.
...
- Expo Router `~57.0.22` mit typed routes
...
| `/admin` | Admin-Session | gemeinsame Reiseorganisation für kleine Zwischenreisen mit den Schritten Zwischenreise & Busse, Gruppen und Live-Begleitung sowie getrennte Punkte für das reiseweite Tagesprogramm, Mehrziel-Navigation, Kontofamilien, Gruppenabfrage, Fragen und Benutzerübersicht |
...
- `trips`: technische Speicherung für aktive oder beendete Zwischenreisen innerhalb der Ziyara; durch einen Partial-Unique-Index höchstens eine aktive Zwischenreise.
...
- `daily_programs`: genau ein veröffentlichter organisatorischer Ablauf je Kalenderdatum für die gesamte Ziyara mit optionaler Überschrift; keine Abhängigkeit von einer Zwischenreise.
- `trip_groups`: benannte Untergruppen der aktiven Zwischenreise mit genau einer kontogebundenen Reisezuordnung als Anführer.
...
Die additive Migration `20260906000000_redesign_bus_trip_assignments.sql` stellt den aktuellen Bus-Adminflow auf registrierte Personen und ganze Kontofamilien um. `trip_participants.participant_code` bleibt unverändert als intern generierter Legacy-Schlüssel bestehen, wird im Client aber weder eingegeben noch angezeigt. `assignment_family_id` markiert eine gemeinsam ausgewählte Familie, während für die statusfähige Kontobindung weiterhin genau ein Zuordnungsdatensatz je betroffenem Profil angelegt wird. `trip_buses.leader_participant_id` hält den registrierten Busführer. Neue Admin-RPCs weisen Personen oder Familien atomar zu, erstellen einen Bus nur zusammen mit einem Führer, ändern Führer und kopieren Busse, Führer und Zuordnungen aus einer beendeten Zwischenreise in eine neue aktive Zwischenreise. Boardingantworten, Eskalationen, Ziele, Reiseführung und Reisegruppen werden bei der Übernahme bewusst nicht kopiert; das Tagesprogramm ist davon vollständig unabhängig. Der Auftraggeber hat am 6. September 2026 gemeldet, den Datenbank-Push ausgeführt zu haben; dieser Status konnte in der Codex-Umgebung wegen des fehlenden Supabase-CLI-Logins nicht unabhängig gelesen werden. Lokal wurde die Migration in diesem Durchlauf nicht ausgeführt, weil der Start der Docker-/Supabase-Umgebung nicht freigegeben wurde.
...
Die additive, lokal und remote angewandte Migration `20260828130000_add_daily_program.sql` führte das Tagesprogramm ursprünglich reisegebunden ein. Die nachfolgende Migration `20260922000000_separate_daily_program_from_intermediate_trips.sql` benennt die Tabelle in `daily_programs` um, entfernt `trip_id`, führt doppelte Kalendertage deterministisch zusammen und ersetzt die RPC durch `admin_upsert_daily_programs(jsonb)`. Damit gilt genau ein Tagesprogramm je Kalenderdatum für die gesamte Ziyara und bleibt beim Anlegen oder Beenden von Zwischenreisen sichtbar. Alle angemeldeten Konten dürfen es lesen; Batch-Upserts sind auf vierzehn unterschiedliche Tage begrenzt und nur Admins erlaubt. Direkte Client-Schreibrechte bleiben entzogen, Realtime bleibt auf der umbenannten Tabelle aktiv. Diese neue Migration muss vor Auslieferung des aktualisierten Clients remote angewandt werden.
...
Die Migrationen `20260903000000_add_emergency_requests.sql`, `20260904000000_add_emergency_dashboard_and_duty.sql` und `20260904010000_require_emergency_location_label.sql` ergänzen Notfallmeldungen, materialisierte rollenbezogene Empfänger, Lesestatus, die frühere Diensteinteilungsstruktur und private Push-Versandversuche. `20260921000000_make_emergency_teams_always_on_duty.sql` ersetzt die Empfängerauswahl: Jede neue Meldung geht immer an alle aktuellen Profile der passenden Teamrolle sowie an alle Admins, und die alte Einteilungs-RPC wird entfernt. Direkte Client-Schreibrechte sind entzogen; Senden und Gelesen-Markierung laufen über authentifizierte RPCs, während Push-Claims und -Abschlüsse ausschließlich `service_role` ausführen darf. RLS zeigt Meldungen nur dem Absender und den beim Absenden festgehaltenen Empfängern. Alle vier Notfallmigrationen sind remote angewandt. `dispatch-emergency-alert` ist der einzige vom aktuellen Client verwendete Notfall-Dispatcher; die remote vorhandene Legacy-Function `dispatch-emergency-duty` wird nicht mehr aufgerufen.
...
Am 22. September 2026 wurden lokale und Remote-Migrationsliste mit der projektlokalen Supabase-CLI erneut gelesen; alle 36 Migrationen waren bis einschließlich `20260922000000` synchron. Der anschließende `db push --linked --include-all --skip-vault --dry-run` meldete keine ausstehenden Migrationen. Keine bestehende Migration wurde verändert, gelöscht oder zusammengefasst. Die Edge Functions `delete-account`, `dispatch-general-alarm`, `dispatch-emergency-alert` und die nicht mehr vom Client verwendete Legacy-Function `dispatch-emergency-duty` wurden remote als `ACTIVE` bestätigt und verwenden ihre jeweilige interne Autorisierungsprüfung. Die Remote-Auth-Redirect-Allowlist wurde nicht verändert.

Für Push ist das EAS-Projekt `@hadi_ea/al-batoul` mit der Projekt-ID aus `app.json` und dem nativen Identifier `de.albatoul.ziyara` verbunden. EAS zeigt einen APNs-Push-Key und einen FCM-V1-Service-Account. Die signierten iOS- und Android-Preview-Builds vom 14. September 2026 sind erfolgreich abgeschlossen, enthalten jedoch nur Commit `2df176f`; der am 21. September geprüfte Stand `aabf955` liegt acht Commits weiter. Zum Prüfzeitpunkt war eine tatsächliche Push-Zustellung auf Geräten weiterhin nicht nachgewiesen. Expo Enhanced Push Security und `EXPO_ACCESS_TOKEN` sind bewusst noch nicht aktiviert. Remote-Zustand kann sich unabhängig vom Repository ändern: vor späteren Annahmen mit autorisiertem Zugriff `npx supabase migration list --linked` und `npx supabase functions list` prüfen. Migrationen, Functions, Secrets, Scheduler und Auth-Redirects nur innerhalb eines ausdrücklich beauftragten Implementierungs- oder Deployment-Schritts remote ändern.

## Kapazität für die Reisegruppe

- Zielgröße sind ungefähr 100 Konten zuzüglich mehrerer Admin-/Mitarbeitergeräte.
- Für diese Größe sind die öffentlichen Rasterkacheln von `tile.openstreetmap.org` für normale, manuell geöffnete Android-Kartenansichten freigegeben. Der Client nutzt die HTTPS-Standard-URL, sichtbare Attribution, die App-Kennung `Ziyara/1.0`, den persistenten HTTP-Cache und lädt weder vorab noch massenhaft offline. Die Karte bleibt eine ergänzende Komfortfunktion; OSM bietet kein SLA und darf nicht als sicherheitskritische Verfügbarkeitsgrundlage behandelt werden. Bei Blockierungen, regelmäßig hoher gleichzeitiger Nutzung oder einer geforderten Verfügbarkeitsgarantie ist auf einen kommerziellen OSM-basierten Tile-Provider mit SLA und austauschbarer URL zu wechseln. Ein eigener Tile-Server ist für etwa 100 Teilnehmer derzeit nicht verhältnismäßig. Gegen den öffentlichen OSM-Dienst darf kein synthetischer Lasttest mit simulierten Geräten ausgeführt werden.
...
- Der Home-Hinweis für ungelesene Notfälle fragt nur bei Konten der Rollen `medical_staff`, `organization_team` und `admin` ab und ergänzt pro gerade geöffneter App höchstens ungefähr einen Fallback-Read pro Minute; normale Teilnehmerkonten erzeugen dafür keine zusätzliche Abfrage.
...
- Das gebündelte Erststartvideo wird mit `expo-video` auf iOS, Android und Web bildschirmfüllend mit Ton in Endlosschleife hinter dem Onboarding abgespielt. Logo und Markenclaim sowie leicht versetzt die Sprachbuttons werden mit kurzen reinen Opacity-Animationen eingeblendet und liegen ohne zusätzliche Abdunklung direkt vor dem Video; ein Sprachbutton speichert die Auswahl und öffnet direkt die Registrierung. Bei einem Wiedergabefehler bleibt der lokale Markenfarb-Hintergrund sichtbar und die Auswahl bedienbar.
...
`npm test` umfasst derzeit 184 Jest-Tests in 39 Suites. Abgedeckt sind unter anderem die gemeinsame Reiseorganisationsübersicht und ihre drei Arbeitsbereiche, der Karten-Ausschnitt für Anführer- und Adminstandort, der validierte Onboardingstatus und seine Navigationsentscheidung, die Familiengruppierung und Summenbildung der Admin-Benutzerliste, AuthContext einschließlich Koffer- und SIM-Karten-Registrierung und -änderung, BusManagementContext einschließlich Session-Refresh-Retry und Fehlerklassifizierung, DailyProgramContext, der validierte benutzergebundene Programm-Cache, lokale Kalenderdatumslogik und die Zerlegung mehrzeiliger Ablaufpunkte, der rollenbezogene Home-Hinweis für ungelesene Notfälle einschließlich Realtime und App-Rückkehr, Generalalarm-Stufen/Erinnerungsplanung und Dispatcher-Autorisierung, TripGuidanceContext einschließlich Offline-Vormerkung, Wiederholung, Problemübernahme und benutzergebundenem Neustart-Cache für Reiseziele, Reisegruppen-Zustandsaufbau, Standortablauf und Fehlerklassifizierung, GroupCheckContext einschließlich des Rollenlade-Timings, QuestionRoundContext, die Navigationsübergabe an Karten-Apps, Koffer- und SIM-Kartenzahlvalidierung, Reader-Präferenzmigration und strukturiertes Absatzmapping, Persistenz-Races und Speicherfehler, der gerenderte `RequireAuth`-Guard, der öffentliche Providerstart einschließlich Reisegruppen ohne Supabase-Zugriff, Recovery-/Account-Löschverträge sowie die globale Error Boundary. `npm run test:coverage` beziehungsweise `npm run validate` erzwingt mindestens 50 % globale Line Coverage sowie jeweils 80 % für Auth-, Busmanagement-, Reiseführungs-, Gruppencheck- und Fragerunden-Kontext. Der vollständig ausgeführte lokale Stand vom 22. September 2026 liegt bei 85,01 % global, 98,24 % für den Notfall-Home-Hinweis, 91,46 % AuthContext, 92,66 % BusManagementContext, 89,77 % DailyProgramContext, 87,79 % TripGuidanceContext, 95,58 % GroupCheckContext und 95,89 % QuestionRoundContext.

Unter `supabase/tests/database` definieren zusätzlich 259 pgTAP-Assertions in vierzehn SQL-Testdateien Prüfungen der RLS-/RPC-/Parallelitätsregeln sowie Cascades, Audit-Anonymisierung, Function-Grants, die automatische vollständige Notfallempfängerauswahl, Kontofamilien, Koffer- und SIM-Karten-Registrierung und -änderung, strukturierte Reader-Absätze, konkurrierende Account-Löschungen, beide Reihenfolgen von Boarding-Antwort gegen Schließung, die serverseitige Generalalarm-Stufenfolge, Token/Versandfenster/Eskalationen, den vollständigen Reiseführungs-Lebenszyklus, das von Zwischenreisen unabhängige atomare Tagesprogramm-Batch und die Reisegruppen-/Anführerstandortgrenzen einschließlich Adminmitgliedschaft und 15-Minuten-Ablauf. `npm run test:e2e` führt neun serielle Playwright-Smokes mit synthetischen Konten gegen die lokale Expo-/Supabase-/Mailpit-Umgebung aus: Registrierung/Login, Recovery-Link, Erststart mit Video, Sprache und Gastzugang bei abgebrochenen Supabase-Requests, Gruppencheck, anonyme Fragerunde, Busmanagement einschließlich der gestuften Generalalarm-Bestätigung, Mehrtagesprogramm von der kompakten Home-Vorschau bis zur Wochenansicht, Reiseführung mit Realtime-Treffpunktänderung und Problemübernahme sowie Rollenänderung. Der E2E-Start liest lokale Schlüssel bei neueren Supabase-CLI-Versionen über `supabase status -o env`, falls `start-secrets/docker.env` nicht erzeugt wird. `.github/workflows/ci.yml` führt bei Pushes und Pull Requests App-Validierung samt Coverage, Expo Doctor, getrennte Web-/iOS-/Android-Exports und einen Critical-Audit-Gate aus; der Datenbank-Job startet das lokale Schema aus Migrationen, prüft die 401-Auth-Gates beider Edge Functions, führt DB-Lint und SQL-Tests sowie danach die Playwright-Smokes aus.

Gezielte Produktionsprüfung vom 21. September 2026 auf Commit `aabf955`: TypeScript bestand; Lint meldete keine Fehler und drei unbenutzte Imports; 176/176 Jest-Tests in 36 Suites bestanden; Web-, iOS- und Android-Export waren erfolgreich. Expo Doctor bestand 20/21 Checks und der Expo-Abhängigkeitscheck schlug fehl, weil `expo`, `@expo/ui`, `expo-constants`, `expo-location`, `expo-notifications` und `expo-router` jeweils eine erwartete SDK-57-Patchversion zurückliegen. `npm audit --omit=dev` meldete 0 Critical, 7 High und 15 Moderate. Die lokale pgTAP-/Playwright-Suite und Coverage wurden in diesem gezielten Durchlauf nicht wiederholt; deren letzter vollständiger Stand bleibt der 14. September 2026 mit 252/252 pgTAP-Assertions und neun Playwright-Smokes.
...
- Android verwendet die öffentlichen Standardkacheln von OpenStreetMap ohne API-Schlüssel oder Zahlungsdaten. Für die erwarteten knapp 100 Teilnehmer ist dies unter den im Abschnitt „Kapazität für die Reisegruppe“ dokumentierten Bedingungen akzeptiert. Fehlende SLA-/Offline-Garantie und die Möglichkeit einer unangekündigten Begrenzung bleiben ein Betriebsrisiko; die Karte darf daher keine sicherheitskritische Voraussetzung sein. Wegen des Android-Autolinking-Ausschlusses von `react-native-maps` ist nach dieser Umstellung ein neuer Android-Build erforderlich.
...
- Die Reisegruppen- und Anführerstandortfunktion ist im Client implementiert und die Migration `20260830000000_add_trip_groups_and_location_requests.sql` ist lokal und remote angewandt. Admins können über ihre kontogebundene Reisezuordnung Mitglied oder Anführer sein. Die iOS-Berechtigungsbeschreibung in `app.json` nennt nun auch die ausdrückliche einmalige Freigabe an die Reiseleitung; diese App-Konfigurationsänderung erfordert einen neuen nativen Development-/Produktionsbuild. Vor produktiver Nutzung fehlen reale Tests von Adminmitgliedschaft, Realtime, Vordergrund-Standortberechtigung, Ablehnung, 15-Minuten-Ablauf und schwacher Verbindung auf iOS, Android und Web.
...
- Das Generalalarm-Schema, die Dispatcher-Function, das Scheduler-Secret, der Supabase-Vault-Eintrag, der Minuten-Cronjob sowie APNs und FCM V1 sind eingerichtet. Die iOS-/Android-Preview-Builds vom 14. September sind fertig, enthalten aber nicht den aktuellen Commit. Vor produktiver Push-Nutzung fehlen aktuelle Builds, Installation, Tokenregistrierung und reale Geräte-/Mobilfunktests. Expo Enhanced Push Security samt `EXPO_ACCESS_TOKEN` bleibt eine optionale spätere Härtung. Details stehen in `docs/GENERAL_ALARM.md`.
- Das Notfallschema, das rollenbasierte Notfall-Dashboard und `20260921000000_make_emergency_teams_always_on_duty.sql` sind remote bereitgestellt. Admins sehen und erhalten beide Notfallarten, medizinisches Personal nur medizinische Fälle und das Organisationsteam die übrigen Anfragen. Bei jeder neuen Notfallmeldung ist eine verständliche textliche Ortsangabe Pflicht; genaue Gerätekoordinaten bleiben freiwillig. Es werden immer alle Konten der passenden Teamrolle und alle Admins adressiert, unabhängig von historischen Diensteinteilungen. Der aktuelle Client bietet keine manuelle Diensteinteilung und ruft nur `dispatch-emergency-alert` auf. Vor produktiver Nutzung fehlen reale Push-Tests beider Zielwege auf iOS und Android, einschließlich mehrerer Empfänger derselben Teamrolle und eines Admin-Kontos. Details stehen in `docs/EMERGENCY_ALERTS.md`.
- Arabisch richtet Texte aus, schaltet aber die gesamte native Layoutreihenfolge noch nicht über `I18nManager` auf RTL um.
- `npm audit --omit=dev` meldet nach den aktuellen SDK-kompatiblen Expo-Patchupdates weiterhin keine Critical-, aber 7 High- und 15 Moderate-Einträge. Sie betreffen transitive Expo-/Metro-Werkzeuge; verbleibende Meldungen einzeln bewerten und keinen `npm audit fix --force` ausführen.
