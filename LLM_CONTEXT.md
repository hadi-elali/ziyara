...
- Beim ersten Start erscheint nach dem nativen Splash Screen direkt die Sprachauswahl, während das lokal gebündelte Einführungsvideo mit Ton in Endlosschleife im Hintergrund läuft. Ein Tipp auf eine Sprache speichert sie und öffnet direkt die Registrierung. Dort bleiben Anmeldung und die Nutzung des öffentlichen Guides ohne Konto als gleichwertige Wege erreichbar; `background-intro.png` bleibt der lokale Fallback bei einem Videofehler.
...
- Das gebündelte Erststartvideo wird mit `expo-video` auf iOS, Android und Web bildschirmfüllend mit Ton in Endlosschleife hinter der sofort sichtbaren Sprachauswahl abgespielt. Logo, Markenclaim und Sprachbuttons liegen ohne zusätzliche Abdunklung direkt vor dem Video; ein Sprachbutton speichert die Auswahl und öffnet direkt die Registrierung. Bei einem Wiedergabefehler bleibt `assets/images/background-intro.png` als Hintergrund sichtbar und die Auswahl bedienbar.
