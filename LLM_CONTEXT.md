...
- Kontofamilien und Kofferanzahl sind im lokalen Client und in der lokalen Datenbank implementiert. Die Migration `20260830010000_add_account_families_and_luggage.sql` ist noch nicht remote ausgerollt. Die SIM-Karten-Erweiterung ist im lokalen Client und als Migration `20260902000000_add_profile_sim_card_count.sql` im Worktree implementiert, wurde auf Wunsch aber nicht weiter getestet oder auf eine Datenbank angewandt. Bis beide Migrationen remote ausgerollt sind, darf der neue Client nicht gegen das Remote-Projekt veröffentlicht werden. Danach fehlen reale Tests von Registrierung, nachträglichen Mengenänderungen, Adminsummen und Familienverschiebung mit mehreren Konten auf kleinen iOS-, Android- und Weboberflächen.
- Das bisherige Busmanagement-Schema ist remote ausgerollt. Den Push der neuen Personen-/Familienzuordnung, verpflichtenden Busführer und Reiseübernahme über `20260906000000_redesign_bus_trip_assignments.sql` hat der Auftraggeber am 6. September 2026 als ausgeführt gemeldet; eine unabhängige Remote-Prüfung war ohne Supabase-CLI-Login nicht möglich. Lokal blieb die Migration mangels freigegebenem Docker-/Supabase-Start ungeprüft. Vor produktiver Nutzung fehlen daher mindestens eine reale Adminprüfung der Familienauswahl und Übernahme sowie weiterhin ein Last-/Mobilfunktest mit der Reisegruppe.
...
- Das Notfallschema, das rollenbasierte Notfall-Dashboard, die Admin-Diensteinteilung sowie `dispatch-emergency-alert` und `dispatch-emergency-duty` sind nur im Worktree. Admins sehen alle Notfälle, medizinisches Personal nur medizinische Fälle und das Organisationsteam die übrigen Anfragen. Bei jeder neuen Notfallmeldung ist eine verständliche textliche Ortsangabe Pflicht; genaue Gerätekoordinaten bleiben freiwillig. Bei vorhandener Diensteinteilung werden nur die eingeteilten Personen adressiert; ohne Einteilung greift ein Fallback auf das gesamte zuständige Team. Vor produktiver Nutzung müssen die Notfallmigrationen remote angewandt, beide Functions bereitgestellt, EAS-Projekt-ID/Push-Credentials konfiguriert und wegen der Android-Kanäle ein neuer nativer Build erstellt werden. Ohne Functions bleiben die internen Meldungen erhalten, aber es gibt keinen Push. Details stehen in `docs/EMERGENCY_ALERTS.md`.
