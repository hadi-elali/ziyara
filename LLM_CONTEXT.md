...
Der letzte vollständige Prüfstand vom 14. September 2026: `npm run validate` bestand mit 182/182 Jest-Tests; Web-, iOS- und Android-Export waren erfolgreich. Expo Doctor bestand 20/21 Checks und meldete ausschließlich verfügbare SDK-57-Patchupdates für sechzehn bereits vorhandene Expo-Pakete. Die vollständige lokale pgTAP-Suite bestand mit 252/252 Assertions. Der DB-Lint meldete weiterhin die bereits vorhandene mehrdeutige `profile_id`-Referenz in `admin_set_emergency_duty` sowie die ungelesene Variable `saved_assignment` in `admin_copy_trip_bus_setup`; die neue Reader-Migration erzeugte keinen zusätzlichen Befund. Die Playwright-E2E-Suite, native Echtgeräteprüfung und ein neuer Development Build wurden in diesem Durchlauf nicht ausgeführt.
...
- Die Veröffentlichungmigration `20260914010000_publish_ziyarat_ashura.sql` ist lokal, aber noch nicht remote angewandt. Bis zum Remote-Rollout bleibt die identische gebündelte Absatzfassung autoritativ; die optionale Serverabfrage fällt ohne Funktionsverlust auf diese Offline-Daten zurück.
