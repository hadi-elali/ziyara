...
- Admins können eigenständige App-Konten zu benannten Kontofamilien zusammenfassen. Ein Profil gehört höchstens einer Kontofamilie; eine neue Zuordnung verschiebt es atomar aus der bisherigen Familie. Diese Zuordnung ist unabhängig von `party_size` und Reisegruppen und kann unter `Reiseorganisation → Reise & Busse` als gemeinsame Zuordnungseinheit verwendet werden.
...
- Der Adminbereich bündelt Reise, Busse, Untergruppen, Reiseführung und Generalalarm im Akkordeon `Reiseorganisation`. Darin bleiben die Schritte `Reise & Busse`, `Gruppen` und `Live-Begleitung` klar getrennt; eine gemeinsame Übersicht zeigt aktive Reise, Busse, Teilnehmer, Gruppen, Reiseführung und Alarmstatus. Im Schritt `Live-Begleitung` legt der Admin für den Generalalarm Meldung und eine frei wählbare Abfahrtszeit zwischen 1 und 1440 Minuten fest. Die Werte 15, 30 und 60 Minuten bleiben als Schnellauswahl verfügbar. Anschließend schaltet der Admin den Alarm ausdrücklich ein, überwacht ihn und beendet ihn wieder.
...
| `/admin` | Admin-Session | gemeinsame Reiseorganisation mit den Schritten Reise & Busse, Gruppen und Live-Begleitung sowie getrennte Punkte für Tagesprogramm, Mehrziel-Navigation, Kontofamilien, Gruppenabfrage, Fragen und Benutzerübersicht |
...
Am 21. September 2026 wurden lokale und Remote-Migrationsliste unabhängig mit der projektlokalen Supabase-CLI gelesen. Alle 34 Migrationen sind bis einschließlich `20260914010000` synchron. Keine bestehende Migration wurde verändert, gelöscht oder zusammengefasst. Die Edge Functions `delete-account`, `dispatch-general-alarm`, `dispatch-emergency-alert` und `dispatch-emergency-duty` wurden remote als `ACTIVE` bestätigt und verwenden ihre jeweilige interne Autorisierungsprüfung. Die Remote-Auth-Redirect-Allowlist wurde nicht verändert.
